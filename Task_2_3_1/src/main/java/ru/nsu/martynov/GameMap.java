...
public class GameMap {
    ...
    private void initMap() {
        ...
        // Создаём змей
        spawnSnakes(settings.getSnakeCount());
    ...
    private void generateWalls() {
        for (int x = 0; x < settings.getWidth(); x++) {
            ...
            walls.add(new Point(x, settings.getHeight() - 1));
        }
        for (int y = 0; y < settings.getHeight(); y++) {
            walls.add(new Point(0, y));
            walls.add(new Point(settings.getWidth() - 1, y));
        }
    }

    private void generateRandomWalls(int percentage) {
        int totalCells = settings.getWidth() * settings.getHeight();
    ...
    private void generateWallsTogether(int percentage) {
        ...
        for (int x = 1; x < settings.getWidth() - 1; x++) {
            for (int y = 1; y < settings.getHeight() - 1; y++) {
    ...
    private void generateConnectedRandomWalls(int percantage) {
        ...
        while (count[0] < targetCount) {
            Point start = new Point(random.nextInt(settings.getWidth()), random.nextInt(settings.getHeight()));
    ...
    private boolean isInsideMap(Point p, Settings settings) {
        return p.getX() > 0 && p.getX() < settings.getWidth() - 1 &&
                p.getY() > 0 && p.getY() < settings.getHeight() - 1;
    }
    ...
    public void spawnSnakes(int count) {
        int pluser = getSnakes().size();
        for (int i = 0; i < count; i++) {
            Point p = getRandomEmptyCell();
            Snake snake = new Snake(p, pluser + i + 1);
            snakes.add(snake);
        }
    }
