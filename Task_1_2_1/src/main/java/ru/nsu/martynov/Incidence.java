...
public class Incidence implements Graph {
    ...
    private void printMatrix() {
        for (int[] integers : matrix) {
            for (Integer integer : integers) {
                // Ensures that positive numbers are displayed with a space before them.
                if (integer >= 0) {
    ...
    public void addVert() {
        ...
        // Copies the existing matrix into the new one
    ...
    public void remVert(int index) {
        ...
        int cnt = 0; // Count of edges not connected to the removed vertex
    ...
    public void addEdge(int from, int to) {
        ...
        if (from == to) {
            ...
            for (int j = 0; j < this.matrix[0].length; j++) {
                if (this.matrix[from][j] > 0 && this.matrix[from][j] == (-1 * this.matrix[to][j])) {
                    edgeIndex = j;
                    break;
                }
            }
    ...
    public int remEdge(int from, int to) {
        Helper.checkIndexes(from, to, this.matrix);
        // Additional logic to remove an edge
        // ...
        return 0; // Adjust return value based on logic
    }
